...
def save_targ_result(outdir, timeout, targ, iter_cnt, triage_vers):
    for triage_ver in triage_vers.split():
        ...
        if not os.path.isfile(RESULT_FILE):
            csv_write_row(RESULT_FILE, ["DIR", "TARGET", "TIMEOUT", "ITER", "TRIAGE_VER"])

        csv_write_row(RESULT_FILE, [os.path.basename(os.path.normpath(outdir)).split("-")[-1], targ ] + tte_list, True)
